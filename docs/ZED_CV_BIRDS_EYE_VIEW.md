...
2. **~~DPO webhook is unprotected~~** — RESOLVED (PARTIAL) by slice 2D-1c (2026-05-09): `POST /api/v1/webhooks/dpo` now has idempotency (refuses to re-process completed transactions) and period-end safety (`max(existing, now) + 30d` instead of unconditional reset). Tier mapping uses exact-price reverse-lookup against `TIER_PRICES`. **Still pending:** signature/HMAC verification — needs DPO Pay docs research; if HMAC unsupported, an nginx IP allowlist on `/webhooks/dpo` is the right answer. Tracked as separate slice.
3. **~~Lenco initiation is live but the webhook handler does not exist~~** — RESOLVED (PARTIAL) by slice 2D-1c (2026-05-09): the live Lenco initiation path in `subscription.py:74-118` was reverted. `services/lenco.py` is preserved on disk for the future restoration slice. Direct calls with `payment_method.startswith("lenco")` now return 422. **Still pending:** building `POST /api/v1/webhooks/lenco` — deferred until Lenco is actually used (zero Lenco payments ever, so zero revenue at risk today).
4. **~~`payments.payment_method` CHECK constraint rejects real values~~** — RESOLVED by slice 2D-1c (2026-05-09): migration `008_relax_payment_method_check.sql` widens the constraint to accept `'card'`, `'card_money'`, and the `'lenco_*'` values. Idempotent guard via `pg_get_constraintdef`. Source-only landing — needs prod-apply at next deploy window (no urgency: `payments` table is currently empty).
...
6. **~~Payment webhook maps tiers by price~~** — RESOLVED by slice 2D-1c (2026-05-09): `webhooks.py` now uses an exact reverse-lookup against `TIER_PRICES` from `app.schemas.subscription` instead of an amount-bucket cascade. Unknown amounts log a warning and fall through to the highest paid tier whose price is ≤ the amount paid (defensive, with audit-trail flag in `webhook_data._inexact_amount_match` for human review). Stricter "reject unknown amounts entirely" deferred until DPO signature verification lands.
...
### High
8. **~~Tier-limit constants diverge across the codebase~~** — RESOLVED by Plan 1 (2026-05-09) + slice 2D-1c (2026-05-09): `app/schemas/subscription.py` is the single source of truth for `TIER_LIMITS` and `TIER_PRICES`. Local divergent dicts in `subscription.py:14` and `admin.py:30` are deleted; webhooks, admin, and subscription paths all import from schemas. New `tests/test_tier_limits.py` pins the canonical values to fail CI loudly on future drift. Constant-extraction follow-up for `auth.py:62,64` (currently hardcoded but matching canonical) tracked separately.
9. **~~Cover letter generation blocked for `super_standard` tier~~** — RESOLVED by Plan 1 (2026-05-09): `cover_letter.py:44` gate now `tier in ("professional", "super_standard")`. Regression-locked by `tests/test_cover_letter.py` covering all four tiers.
10. **~~`POST /api/v1/jobs` is open to any authenticated user~~** — RESOLVED by Plan 2 (2026-05-09): swapped `Depends(get_current_user_id)` for `Depends(require_admin)` (admin/superadmin only). Existing happy-path tests updated to seed `users.role='admin'`; new `test_create_job_forbidden_for_non_admin` regression-locks the gate.
11. **~~OTP `attempts` counter is never incremented and `/auth/otp/verify` has no rate limit~~** — RESOLVED by Plan 2 (2026-05-09): `verify_otp` now has `@limiter.limit("10/minute")` and increments `otp_codes.attempts` on the wrong-code path before raising 401. Existing dead-code attempts check at the top of the function now actually fires. New `test_verify_invalid_code_increments_attempts` regression-locks both the increment and the rate-limit. (Bonus Fix 4 — OTP plaintext hashing — explicitly deferred to its own slice; deploy-window invalidation makes it more than a one-line change.)
...
14. **~~CV filename used as storage path without sanitization~~** — RESOLVED by Plan 2 (2026-05-09): added `_safe_filename()` helper in `cv.py` that strips path components via `os.path.basename`, regex-replaces non-`[A-Za-z0-9._-]` chars with `_`, strips leading dots, caps stem at 80 chars and ext at 10 chars, falls back to `cv-{uuid4}` for empty input. Defense in depth: even if any layer misses, the others catch.
...
20. **~~No error tracking~~** — RESOLVED (2026-05-10): Sentry wired into FastAPI backend via `sentry-sdk[fastapi]`. Project `zedcv-backend` in org `convergeo-w2` (de.sentry.io). Initialized in `main.py` BEFORE `FastAPI()` so the ASGI integration can hook the middleware stack. `traces_sample_rate=0.1`, `send_default_pii=False` (no phone numbers / emails / IPs leak to Sentry). Init is a no-op when `SENTRY_DSN` is empty — local dev stays Sentry-free. DSN must be set as a deploy-time env var on the OCI host (`~/n8n-docker/.env`), never committed.
