...
class TestJobList:
    ...
    def test_list_jobs_with_results(self, client, auth_headers, fake_supabase):
        """Returns formatted jobs with skills."""
        fake_supabase.set_table("jobs", FakeSupabaseQuery(
            data=[{
                "id": "job-1", "title": "Python Developer",
                "company": "TechCo", "location": "Lusaka",
                "description": "Build APIs", "source": "manual",
                "posted_at": "2025-01-01T00:00:00Z", "is_active": True,
                "job_skills": [
                    {"skills": {"name": "python"}},
                    {"skills": {"name": "fastapi"}},
                ],
        ...
        assert "python" in body["jobs"][0]["skills"]


class TestJobCreate:
    @patch("app.api.v1.jobs.generate_embedding", new_callable=AsyncMock)
    def test_create_job_success(self, mock_embed, client, auth_headers, fake_supabase):
        """Creates a job with embedding and dedup fingerprint."""
        mock_embed.return_value = [0.1] * 1536
        fake_supabase.set_table("job_fingerprints", FakeSupabaseQuery(data=[]))
        # Mock must include all fields that Supabase returns (including DB defaults)
        fake_supabase.set_table("jobs", FakeSupabaseQuery(data=[{
            "id": "job-new", "title": "React Developer Needed",
            "company": "StartupX", "description": "Build modern UIs with React and TypeScript for our platform",
            "location": "Kitwe", "source": "manual",
            "posted_at": "2025-01-01T00:00:00Z", "is_active": True,
            "embedding": [0.1] * 1536,
        }]))
    ...
    @patch("app.api.v1.jobs.generate_embedding", new_callable=AsyncMock)
    def test_create_job_duplicate_rejected(self, mock_embed, client, auth_headers, fake_supabase):
        """Rejects duplicate jobs based on fingerprint."""
        mock_embed.return_value = [0.1] * 1536
        fake_supabase.set_table("job_fingerprints", FakeSupabaseQuery(
            data=[{"job_id": "existing-job"}]
        ))
        resp = client.post("/api/v1/jobs", headers=auth_headers, json={
            "title": "React Developer Needed",
            "company": "StartupX",
