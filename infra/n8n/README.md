...
### Normalize drops `source_url` even when Combine has URLs

If **Combine All Sources** shows per-job URLs (e.g. `gozambiajobs.com/jobs/481442576-…`) but **Normalize and Deduplicate** outputs `source_url: null` for every row, the live node likely still uses an old `normSourceUrl` that rejects **all** aggregator hosts (not just homepages). Replace the node with `infra/n8n/snippets/normalize_and_deduplicate.js` — it mirrors backend `sanitize_listing_source_url` (keep `/jobs/<slug>`, drop `/` and `/jobs` index only). It also keeps LinkedIn rows that have `source_url` but `description: null`.

>>>>>>> 6d591bd (fix(n8n): preserve LLM source_url in normalizer (aggregator listing paths))
